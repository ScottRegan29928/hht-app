...
export const siteOrigin = internalQuery({
  ...
  handler: async (ctx, { siteSlug }) => {
    ...
    return {
      ...
      // `domain` is the FUTURE production hostname and still points at the old
      // WordPress site until cutover, so it cannot be trusted for a link an
      // owner has to click today. Candidates are probed in resolveHost below.
...
/**
 * Pick a hostname that actually serves this app right now.
 *
 * Before cutover the canonical domain is still WordPress, which would 404 the
 * activation link; after cutover it is the right answer. Rather than hardcode
 * either, ask each candidate for /robots.txt and take the first one this app
 * answers. Only hostnames already stored on the site record are considered, so
 * this cannot be pushed somewhere arbitrary.
 */
async function resolveHost(site: {
  domain: string;
  altDomains: string[];
}): Promise<string> {
  const candidates = [site.domain, ...site.altDomains];
  for (const host of candidates) {
    try {
      ...
      // Our robots.txt is generated by convex/http.ts and always has this line.
      if (body.includes("Disallow: /owner")) return host;
    } catch {
      // Unreachable candidate: try the next one.
    }
  }
  // Nothing answered as ours — fall back to the lead-works host if there is
  // one, since that is where the app lives pre-cutover.
  return (
    site.altDomains.find((h) => h.endsWith("lead-works.com")) ?? site.domain
  );
}
...
export const sendInvite = action({
  ...
  handler: async (
    ...
    const host = await resolveHost(site);
    const link = `https://${host}/owneractivate?token=${invite.token}`;
